...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Clock, Plus, AlertCircle, X, Check, Trash2 } from 'lucide-react';
...
export const Calendar: React.FC<CalendarProps> = ({ reminders, isLoading, user, onAddLocalReminder, onDeleteLocalReminder }) => {
  ...
  const [eventType, setEventType] = useState<'exam' | 'attention' | 'other'>('other');
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (response.ok) {
              ...
              // alert('Событие успешно сохранено в базу данных!'); 
          } else {
              throw new Error('Failed to save to server');
          }
      } catch (error) {
          console.error("Server error, saving locally for session:", error);
          // Fallback: Create a local object so the UI updates even if server is down (Demo mode)
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < startDay; i++) {
      days.push(
        <div key={`empty-${i}`} className="min-h-[6rem] bg-slate-50/30 rounded-xl border border-transparent"></div>
      );
    }
            ...
            >
                ...
                <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-1 mt-1">
                        {hasExam && <div className="w-2 h-2 rounded-full bg-red-500 shadow-sm shadow-red-200" title="Экзамен"></div>}
                        {hasAttention && <div className="w-2 h-2 rounded-full bg-amber-400 shadow-sm shadow-amber-200" title="Важно"></div>}
                        {!hasExam && !hasAttention && dayReminders.length > 0 && <div className="w-2 h-2 rounded-full bg-blue-400 shadow-sm shadow-blue-200"></div>}
                    </div>
                </div>
                
                <div className="mt-auto space-y-1">
                   {dayReminders.slice(0, 2).map((rem, idx) => (
                       <div key={idx} className={`text-[9px] font-bold truncate px-1.5 py-0.5 rounded-md ${
                           rem.category === 'exam' ? 'bg-red-50 text-red-600' :
                           rem.category === 'attention' ? 'bg-amber-50 text-amber-600' :
                   ...
                   {dayReminders.length > 2 && (
                       <div className="text-[9px] text-slate-400 pl-1 font-bold">+ еще {dayReminders.length - 2}</div>
                   )}
                </div>
            </div>
        );
  ...
  return (
    <div className="flex gap-4 h-full overflow-hidden relative">
        ...
        <div className="hidden lg:flex w-80 bg-white rounded-3xl p-6 flex-col shadow-sm border border-slate-100 shrink-0 h-full overflow-hidden">
            <div className="mb-6 shrink-0 flex justify-between items-end">
                <div>
                    <h2 className="text-lg font-bold font-mono text-slate-800 mb-1">События</h2>
                    <p className="text-xs text-slate-400">Синхронизировано с базой</p>
                </div>
                {isLoading && <div className="w-4 h-4 rounded-full border-2 border-blue-500 border-t-transparent animate-spin mb-2"></div>}
            </div>
            ...
            {selectedDate ? (
                        ...
                        {selectedReminders.length > 0 ? (
                            <div className="space-y-3 pb-4">
                                {selectedReminders.map(rem => (
                                    <div key={rem.id} className="bg-white border border-slate-100 p-4 rounded-xl shadow-sm hover:shadow-md transition-all group relative">
                                        <div className="flex justify-between items-start mb-2 pr-6">
                                            <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded ${
                                                ...
                                                {rem.category === 'exam' ? 'Экзамен' : rem.category === 'attention' ? 'Важно' : 'Заметка'}
                                            </span>
                                        </div>
                                        ...
                                        <h4 className="font-bold text-slate-800 text-sm mb-1">{rem.title}</h4>
                                        {rem.description && <p className="text-xs text-slate-500 leading-relaxed mb-2">{rem.description}</p>}
                                        <div className="flex items-center justify-between pt-2 border-t border-slate-50 mt-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] font-bold text-slate-400">{rem.teacherName}</span>
                                            </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center h-full">
                     ...
                     <p className="text-xs text-slate-400 mt-2 max-w-[180px] leading-relaxed">Нажмите на любой день в календаре, чтобы увидеть расписание и заметки.</p>
                </div>
            )}
        </div>
        ...
        <div className="flex-1 bg-white rounded-3xl p-6 shadow-sm overflow-hidden flex flex-col border border-slate-100">
            <div className="flex justify-between items-center mb-6 shrink-0">
                <div className="flex gap-3 items-center">
                    ...
                    <div className="flex items-baseline gap-3 justify-center px-4 min-w-[220px]">
                        <h2 className="text-2xl font-black font-mono text-slate-800 tracking-tight leading-none">
                            {monthNames[currentDate.getMonth()]}
                        </h2>
                        <span className="text-xl font-bold text-slate-400 tracking-widest">{currentDate.getFullYear()}</span>
                    </div>
                    <button onClick={() => changeMonth(1)} className="w-10 h-10 flex items-center justify-center hover:bg-slate-100 rounded-xl transition-colors bg-white border border-slate-200 text-slate-600 shadow-sm active:scale-95">
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar overflow-x-hidden">
                <div key={currentDate.toISOString()} className={`grid grid-cols-7 gap-2 pb-2 ${animClass}`}>
                    {renderCalendarDays()}
                </div>
            </div>
        </div>
        ...
        {isModalOpen && selectedDate && (
             <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm animate-fadeIn">
                 <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm border border-slate-200 overflow-hidden animate-slideUp">
                    <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <h3 className="font-bold text-slate-800">Новое событие</h3>
                        <button onClick={() => setIsModalOpen(false)} className="p-1.5 hover:bg-slate-200 rounded-full text-slate-400 hover:text-slate-600 transition-colors">
                            <X size={20} />
                        </button>
                    </div>
                    <div className="px-5 py-3 bg-blue-50/50 border-b border-blue-100 flex items-center gap-3">
                        <div className="bg-blue-100 text-blue-600 px-2 py-1 rounded text-xs font-bold font-mono">
                    ...
                    <form onSubmit={handleCreateEvent} className="p-6 space-y-4">
                        <div className="space-y-1">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Тип события</label>
                            <div className="flex gap-2">
                                <button 
                                    type="button" 
                                    onClick={() => setEventType('exam')}
                                    className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase transition-all ${eventType === 'exam' ? 'bg-red-500 text-white shadow-md shadow-red-200' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}
                                >
                                    Экзамен
                                </button>
                                <button 
                                    ...
                                    className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase transition-all ${eventType === 'attention' ? 'bg-amber-400 text-white shadow-md shadow-amber-200' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}
                                >
                                    Важно
                                </button>
                                <button 
                                    ...
                                    className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase transition-all ${eventType === 'other' ? 'bg-blue-500 text-white shadow-md shadow-blue-200' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}
                                >
                                    Заметка
                                </button>
                            </div>
                        ...
                        <div className="space-y-1">
                             <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Заголовок</label>
                             <input 
                                type="text"
                                value={eventTitle}
                        ...
                        <div className="space-y-1">
                             <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Описание (опционально)</label>
                             <textarea 
                                value={eventDesc}
