...
export const CourseStudents: React.FC<CourseStudentsProps> = ({ onBack }) => {
    const students = [
        ...
        { id: 3, name: 'Дмитрий Сидоров', email: 'dmitry@example.com', progress: 45 },
        { id: 4, name: 'Елена Козлова', email: 'elena@example.com', progress: 78 },
        { id: 5, name: 'Николай Волков', email: 'nikolay@example.com', progress: 60 },
    ];
    ...
    return (
        <div className="bg-white rounded-3xl p-8 shadow-sm h-full flex flex-col">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h2 className="text-2xl font-mono font-bold text-slate-800">Студенты курса</h2>
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm w-64 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                    />
                </div>
                <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                    Всего студентов: <span className="text-slate-800">{students.length}</span>
                </div>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                        {students.map((student) => (
                            <tr key={student.id} className="border-b border-slate-100 last:border-0 hover:bg-white transition-colors group">
                                ...
                                <td className="p-4">
                                    <div className="flex items-center gap-2">
                                        <div className="w-24 h-2 bg-slate-200 rounded-full overflow-hidden">
                                            <div 
                                                className={`h-full rounded-full ${student.progress > 80 ? 'bg-green-500' : student.progress > 50 ? 'bg-blue-500' : 'bg-amber-400'}`} 
