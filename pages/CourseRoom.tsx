...
export const CourseRoom: React.FC<CourseRoomProps> = ({
  courseId,
  user,
  onBack,
}) => {
  ...
  useEffect(() => {
    ...
    socket.on(
      "offer",
      async (payload: { sdp: RTCSessionDescriptionInit; caller: string }) => {
        if (isTeacher) return;
    ...
    socket.on(
      "ice-candidate",
      async (payload: { candidate: RTCIceCandidateInit; caller: string }) => {
        const pc = peerConnectionsRef.current.get(payload.caller);
        if (pc) {
  ...
  // ... [WebRTC Logic functions same as before] ...
  ...
  const stopScreenShare = async () => {
    ...
    if (socketRef.current) socketRef.current.emit("sharing-stopped", courseId);
    peerConnectionsRef.current.forEach((pc) => pc.close());
    peerConnectionsRef.current.clear();
    await fetch(`${API_URL}/api/room/${courseId}/update`, {
      method: "POST",
  ...
  if (!roomState.isActive && !isTeacher) {
    return (
      <div className="bg-white rounded-3xl p-8 shadow-sm h-full flex flex-col relative overflow-hidden">
        <button
          onClick={onBack}
          className="absolute top-8 left-8 p-2 rounded-xl bg-slate-100 hover:bg-slate-200 transition-colors"
        >
          <ChevronLeft size={20} />
        </button>
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-slate-500 max-w-md">
            Комната откроется автоматически, как только занятие начнется.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-slate-900 rounded-3xl overflow-hidden h-full flex flex-col shadow-2xl relative">
      {/* Header */}
      <div className="bg-slate-800/50 backdrop-blur-md p-3 md:p-4 flex justify-between items-center border-b border-slate-700">
        <div className="flex items-center gap-3 md:gap-4">
          <button
            onClick={onBack}
            className="bg-slate-700 hover:bg-slate-600 text-slate-300 px-3 py-2 md:px-4 rounded-lg text-xs font-bold uppercase flex items-center gap-2"
          >
            <LogOut size={16} /> <span className="hidden md:inline">Выйти</span>
          </button>
          <div>
            <h2 className="font-bold text-white text-xs md:text-sm max-w-[150px] md:max-w-none truncate">
              {course?.title}
            </h2>
        ...
        {isTeacher ? (
          <div className="flex gap-2">
            ...
            <button
              onClick={endRoom}
              className="bg-red-600 hover:bg-red-700 text-white px-3 py-2 md:px-4 rounded-lg text-xs font-bold uppercase flex items-center gap-2"
            >
              <Power size={16} />{" "}
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="flex-1 bg-black relative flex flex-col items-center justify-center overflow-hidden min-h-[40vh] lg:min-h-auto">
          {/* TEACHER: Local Preview Overlay */}
          {isTeacher && isSharingScreen && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10">
              <p className="text-white bg-red-600 px-4 py-2 rounded-full text-xs md:text-sm font-bold animate-pulse shadow-lg whitespace-nowrap">
          ...
          {isSharingScreen && isTeacher && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-10">
              <button
                ...
                className="bg-red-600/90 backdrop-blur hover:bg-red-700 text-white px-6 py-2 rounded-full text-xs font-bold uppercase shadow-lg"
              >
                Остановить
              </button>
            </div>
          ...
          <video
            ...
            className={`w-full h-full object-contain ${
              roomState.isScreenSharing && !isTeacher ? "block" : "hidden"
            }`}
          />
            ...
            <div className="flex flex-col items-center justify-center text-slate-500 p-4 text-center">
              ...
              {isTeacher && (
                <button
                  ...
                  className="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold uppercase text-xs flex items-center gap-2 transition-all"
                >
        ...
        {/* Chat Panel */}
        <div className="h-[40vh] lg:h-full lg:w-80 bg-slate-800 border-t lg:border-t-0 lg:border-l border-slate-700 flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
            ...
            {messages.map((msg, idx) => {
              ...
              const isMsgTeacher =
                msg.role === Role.TEACHER || msg.role === Role.ADMIN;
              return (
                <div
                  key={idx}
                ...
                >
                  <div className="flex items-baseline gap-2 mb-1">
                    <span
                      className={`text-[10px] font-bold ${
                        isMsgTeacher ? "text-blue-400" : "text-slate-400"
                      }`}
                    >
                      {msg.username}
                    </span>
                  ...
                  <div
                    className={`px-3 py-2 rounded-xl text-xs max-w-[90%] break-words leading-relaxed ${
                      isMe
            ...
            {roomState.isChatLocked && !isTeacher ? (
              ...
              <form onSubmit={handleSendMessage} className="relative">
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors disabled:opacity-50"
                >
                  <Send size={16} />
                </button>
              </form>
        ...
        <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-4">
          <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 text-center max-w-md w-full shadow-2xl">
            ...
            <div className="flex gap-4 justify-center mt-6">
              <button
                onClick={onBack}
                className="px-6 py-3 rounded-xl bg-slate-700 text-slate-300 font-bold uppercase text-xs hover:bg-slate-600 flex-1"
              >
                Отмена
              </button>
              <button
                onClick={startRoom}
                className="px-8 py-3 rounded-xl bg-blue-600 text-white font-bold uppercase text-xs hover:bg-blue-500 shadow-lg flex-1"
              >
                Открыть
              </button>
            </div>
