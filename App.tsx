...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
        if (currentUser && !currentUser.groupId) {
             try {
                const res = await fetch(`${API_URL}/api/settings/${currentUser.username}`);
                if (res.ok) {
                    const data = await res.json();
                    if (data.groupId) {
                        // This update will trigger the persistence effect above
                        setCurrentUser(prev => prev ? ({...prev, groupId: data.groupId}) : null);
                    }
                }
             } catch (e) {
  ...
  const handleCourseSelect = (courseId: string) => {
      ...
      // View transition is handled by calling setCurrentView explicitly in Courses.tsx
  };
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case View.MATERIALS:
        return <Materials />;
      case View.COURSES:
        return <Courses onNavigate={setCurrentView} user={currentUser} onCourseSelect={handleCourseSelect} />;
      case View.COURSE_DETAILS:
        if (!selectedCourseId) return <Courses onNavigate={setCurrentView} user={currentUser} onCourseSelect={handleCourseSelect} />;
        return <CourseDetails courseId={selectedCourseId} user={currentUser} onBack={() => setCurrentView(View.COURSES)} />;
      case View.COURSE_ROOM:
        if (!selectedCourseId) return <Courses onNavigate={setCurrentView} user={currentUser} onCourseSelect={handleCourseSelect} />;
        return <CourseRoom courseId={selectedCourseId} user={currentUser} onBack={() => setCurrentView(View.COURSES)} />;
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-[1920px] mx-auto overflow-hidden h-screen">
      <Header 
        user={currentUser} 
      ...
      />
      <div className="flex-1 flex px-4 pb-4 gap-4 overflow-hidden">
        <Sidebar 
